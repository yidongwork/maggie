...
public class Segment{ //declare a class name Segment
	...
	public static boolean isIntersected(Segment s1, Segment s2){
		Point2D pt1 = s1.startPoint;
		Point2D pt2 = s1.endPoint;
		Point2D pt3 = s2.startPoint;
		Point2D pt4 = s2.endPoint;
		...
		Segment segmentB = new Segment (pt3, pt4);
...
//			System.out.println("There is no intersection"); //print out "There is no intersection"
			return false; //return the if condition
		} 
		Point2D resultPoint = Segment.calculateIntersection(segmentA, segmentB);
		if (Segment.isOn(segmentA, resultPoint) && Segment.isOn(segmentB, resultPoint)) {
			return true;
		}
		else return false;
	}
